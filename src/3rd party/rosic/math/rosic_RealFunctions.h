...
#include "rosic_CephesDeclarations.h"
...
namespace rosic
{
  ...
  1...31 will be an access violation. These numbers are useful for making Taylor series 
  approximations. */
  static long double inverseFactorials[32] =
  {
    1.0,
    1.0,
    1.0/2.0,
  ...
  INLINE double binomialCoefficient(double n, int k);
  ...
  /** Returns the value of the Jacobian elliptic function cd(u,m) with argunment u and parameter m 
  ( m=k^2 with k being elliptic modulus (?) ) . */
  INLINE double cd(double u, double m);

  /** Returns the value of the Jacobian elliptic function cn(u,m) with argunment u and parameter m 
  ( m=k^2 with k being elliptic modulus (?) ) . */
  INLINE double cn(double u, double m);

  /** Approximates cos^2(0.5*pi*x) over the domain -1.0...+1.0 via
  y = a*(1-x^2)^2 + (1-a)*(1-x^2)^3 with a == 0.565... (found visually - may be refined via
  optimization). */
  INLINE double cosSquaredApprox(double x);

  /** Returns the value of the Jacobian elliptic function dn(u,m) with argunment u and parameter m 
  (m=k^2 with k being elliptic modulus (?) ) . */
  INLINE double dn(double u, double m);

  /** Evaluates the Jacobian elliptic functions sn(u|m), cn(u|m), and dn(u|m) of parameter m
  between 0 and 1, and real argument u. phi is the amplitdue of u. */
  INLINE int ellipticFunctions(double u, double m,
  ...
  /** limits the value of an object.   ...redunadant with clip */
  template <class T>
  ...
  distribution. k<2 makes the distribution more spikey and the tails fatter, values k>2 make the 
  distribution more rectangular. */
  INLINE double minkowski(double x, double mu, double sigma, double k);

  /** Calculates the power of the absolute-value of some number and re-applies
  its original sign afterwards, if base==0, the result will be 0 also. */
  INLINE double powBipolar(double base, double exponent);

  /** Power function for intergers. Multiplies the base exponent-1 time with itself and
  returns the result. */
  ...
  amount of -2*d is compesated such that the resulting waveform is DC-less. */
  INLINE double pulseWave(double x, double d);

  /** Quantizes x to the interval i. */
  INLINE double quant(double x, double i);

  /** Quantizes x (assumed to be in -1.0 - 1.0) to a grid which represents a resolution of nBits 
  bits. */
  ...
  /** Calculates hyperbloic sine and cosine of x -this is more efficient than calling sinh(x)
  and cosh(x) seperately. */
  INLINE void sinhCosh(double x, double* sinhResult, double* coshResult);

  /** Returns the value of the Jacobian elliptic function sn(u,m) with argunment u and
  parameter m ( m=k^2 with k being elliptic modulus (?) ) . */
  INLINE double sn(double u, double m);
  ...
  INLINE double acosh(double x)
  {
    ...
    {
      DEBUG_BREAK; // argument must be >= 1, for the general case use the complex version acosh_c
                   // defined in rosic_ComplexFunctions.h
      return 0.0;
    }
  }
  ...
  INLINE double acoshs(double x)
  {
    ...
    {
      DEBUG_BREAK; // fabs(x) must be >= 1, for the general case use the complex version acosh_c
                   // defined in rosic_ComplexFunctions.h
      return 0.0;
    }
  }
  ...
  /*
  INLINE double besselj(double order, double x)
  {
  double vm;
  double *jv, *yv, *djv, *dyv;

  bessjyv(order, x, vm, jv, yv, djv, dyv);
  ...
  */
  ...
  INLINE double cosSquaredApprox(double x)
  {
    ...
    //double a = 0.5656253;     // found visually - maybe refined via optimization
