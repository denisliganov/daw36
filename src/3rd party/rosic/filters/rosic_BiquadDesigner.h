...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    method. */
    static INLINE void calculateFirstOrderHighpassCoeffsBilinear(double& b0, double& b1, double& b2,
      double& a1, double& a2, const double& oneOverSampleRate, const double& frequency);
    ...
    /** Calculates the coefficients for a two-pole bandpass type of filter with constant skirt
    gain according to Robert Bristow Johnson's Cookbook formula. */
    static INLINE void calculateCookbookBandpassConstSkirtCoeffsViaBandwidth(double& b0, double& b1,
      double& b2, double& a1, double& a2, const double& oneOverSampleRate,
    ...
    /** Calculates the coefficients for a first order allpass filter (according to the
    formula from DAFX?). */
