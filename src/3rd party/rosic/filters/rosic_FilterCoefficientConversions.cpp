#include "rosic_FilterCoefficientConversions.h"
using namespace rosic;

void rosic::directFormToLatticeFir(double *directFormCoeffs, int order, double *reflectionCoeffs)
{
...
void rosic::latticeToDirectFormFir(double *reflectionCoeffs, int order, double *directFormCoeffs)
{
  int     N    = order;            // number of direct-form and reflection coefficients
  ...
  // to scale the FIR vector with this factor and also use it as leading coefficient, we also need to
  // invert the sign:
  for(i=1; i<=N; i++)
    a[i] = -a[i] * k[0];
  a[0] = k[0];
...
void rosic::polesAndZerosToBiquadCascade(Complex *poles, int numPoles, 
                                         Complex *zeros, int numZeros, 
...
void rosic::polesAndZerosToBiquadCascade(Complex *poles, Complex *zeros, int order,
...
void rosic::biquadCascadeToDirectForm(int numBiquads, double *b0, double *b1, double *b2, 
...
double rosic::getBiquadMagnitudeAt(double b0, double b1, double b2, double a1, double a2, 
                                   double omega)
{
...
void rosic::normalizeBiquadStages(double *b0, double *b1, double *b2, double *a1, double *a2, 
