...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    { 
      filter1.setSampleRate(newSampleRate, updateCoefficients);
      filter2.setSampleRate(newSampleRate, updateCoefficients);
    }

    /** Sets the mode for the first filter. @see: TwoPoleFilter::modes. */
    void setMode1(int newMode, bool updateCoefficients = true)
    { filter1.setMode(newMode, updateCoefficients); }

    /** Sets the mode for the second filter. @see: TwoPoleFilter::modes. */
    void setMode2(int newMode, bool updateCoefficients = true)
    { filter2.setMode(newMode, updateCoefficients); }

    /** Sets the frequency for the first filter in Hz. */
    void setFrequency1(double newFrequency, bool updateCoefficients = true)
    { freq1 = newFrequency; filter1.setFrequency(freqScale*freq1, updateCoefficients); }

    /** Sets the frequency for the second filter in Hz. */
    void setFrequency2(double newFrequency, bool updateCoefficients = true)
    { freq2 = newFrequency; filter2.setFrequency(freqScale*freq2, updateCoefficients); }

    /** Sets a scale factor to scale the frequencies of both filters. */
    void setFrequencyScale(double newScale)
    {
      freqScale = newScale; 
      filter1.setFrequency(freqScale*freq1, true);    
      filter2.setFrequency(freqScale*freq2, true);
    }

    /** Sets the boost/cut gain in dB for the first filter. */
    void setGain1(double newGain, bool updateCoefficients = true)
    { gain1 = newGain; filter1.setGain(gainScale*gain1, updateCoefficients); }

    /** Sets the boost/cut gain in dB for the second filter. */
    void setGain2(double newGain, bool updateCoefficients = true)
    { gain2 = newGain; filter2.setGain(gainScale*gain2, updateCoefficients); }

    /** Sets a scale factor to scale the gains of both filters. */
    void setGainScale(double newScale)
    {
      gainScale = newScale; 
      filter1.setGain(gainScale*gain1, true);    
      filter2.setGain(gainScale*gain2, true);
    }

    /** Sets the bandwidth in octaves for the first filter. */
    void setBandwidth1(double newBandwidth, bool updateCoefficients = true)
    { 
      bandwidth1 = newBandwidth; 
      filter1.setBandwidth(bandwidthScale*bandwidth1, updateCoefficients); 
    }

    /** Sets the bandwidth in octaves for the second filter. */
    void setBandwidth2(double newBandwidth, bool updateCoefficients = true)
    { 
      bandwidth2 = newBandwidth; 
      filter2.setBandwidth(bandwidthScale*bandwidth2, updateCoefficients); 
    }
    ...
    void setBandwidthScale(double newScale)
    {
      ...
      filter1.setBandwidth(bandwidthScale*bandwidth1, true);    
      filter2.setBandwidth(bandwidthScale*bandwidth2, true);
    }
