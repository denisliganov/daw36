    ...
    if (key != 0)
    {
        ...
        else
        {
            switch(key)
            {
                ...
                case key_f2:
                {
                    SubWindow* wc = MObject->getWindow()->addWindow(new ConfigWinObject());
                ...
                case key_f8:
                {
                    SubWindow* wc = MObject->getWindow()->addWindow(new MakeUpObject());
...
void TextCursor::advanceView(float dtick, int dline)
{
    if(dtick != 0)
    {
        grid->hscr->setOffset(grid->hscr->getOffset() + dtick);
    }
